...
export default async function(req) {
    ...
    try {
        logger.error(`Starting (GET)/filas/${digitalQueueId}`);
        ...
        const template = fs.readFileSync(`${PUBLIC_PATH}/templates/pub-digital-queue.html`, {encoding: 'utf8'})
            .replace(/<server\.digitalQueueId ?\/?>/gm, digitalQueueId)
            .replace(/<server\.digitalQueueName ?\/?>/gm, queryResult[0].name);

        logger.error(`Ending (GET)/filas/${digitalQueueId}`);
        ...
        logger.error(`Error in (GET)/filas/${digitalQueueId}`);
        logger.info(error);
